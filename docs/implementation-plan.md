...
| M4 Aider + vLLM 验收 | 未通过（Aider 出站网络阻塞，任务未到推理） | 固定任务及隔离运行办法见 `docs/m4-aider-acceptance.md`。2026-09-26 资源空闲后，专用 vLLM 0.29.0 成功加载既有模型；唯一一次固定 Aider 0.86.2 任务卡在 `huggingface.co:443` HTTPS 超时，退出码 143。vLLM 收到 0 次 chat completion；45 条 OTLP span 均为启动/加载 span，`llm_request` 为 0，Aider 测试未执行，故没有 Agent 效果或 live 请求关联证据。服务已停止、GPU 恢复空闲；未改产品代码。全套本地测试此前为 105 项及 3 个子用例通过，已保存脱敏样本检查符合 UNKNOWN 与 scope 边界，但均不替代 M4。 |
