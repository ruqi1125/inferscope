...
| M4 Aider + vLLM 验收 | 未通过（固定任务上下文超出模型 8K 上限） | 固定任务及隔离运行办法见 `docs/m4-aider-acceptance.md`。首次直连 HF 超时；按用户建议设置 `HF_ENDPOINT=https://hf-mirror.com` 后，Aider 通过镜像下载其 `Xenova/llama-3-tokenizer`（约 9.1 MB），无需重下现有权重。Aider 0.86.2 在 vLLM 0.29.0/Llama 3 8B 上实际产生 4 条成功 `llm_request` trace（可追溯 ID、prompt/completion tokens、TTFT/E2E 见验收记录），另有 3 次 HTTP 400。即使按模型配置支持的 8,192 context 启动，Aider 自动加入相关文件后仍估算 8,823 输入 tokens，任务未修改产品文件，`pytest -q` 未运行；Aider 返回码 0 不代表验收通过。所有临时环境、缓存和服务已清理，GPU 释放；没有源代码缺陷证据。下一步需在固定任务/模型不变下决定如何调整 Aider 内置上下文预算；当前本地完整测试基线 105 项及 3 个子用例通过，不能替代 M4。 |
