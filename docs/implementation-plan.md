...
| M4 Aider + vLLM 验收 | 未通过（Llama 3 固定任务上下文超出 8K；Qwen 重跑因 GPU 被占用而未进入 Aider） | 固定任务及隔离运行办法见 `docs/m4-aider-acceptance.md`。首次直连 HF 超时；按用户建议设置 `HF_ENDPOINT=https://hf-mirror.com` 后，Aider 通过镜像下载其 `Xenova/llama-3-tokenizer`（约 9.1 MB），无需重下现有权重。Aider 0.86.2 在 vLLM 0.29.0/Llama 3 8B 上实际产生 4 条成功 `llm_request` trace（可追溯 ID、prompt/completion tokens、TTFT/E2E 见验收记录），另有 3 次 HTTP 400。即使按模型配置支持的 8,192 context 启动，Aider 自动加入相关文件后仍估算 8,823 输入 tokens，任务未修改产品文件，`pytest -q` 未运行；Aider 返回码 0 不代表验收通过。随后按用户授权准备使用现有 Qwen3.6-27B-GPTQ-Int4，未下载权重；准备期间另一项任务占用两张 GPU，vLLM 启动因 cuda:1 只剩 8.66 GiB（预算需 20.02 GiB）退出，OTLP 仅收到 10 条初始化 span、0 条 `llm_request`，Aider 与自动测试未运行；未干预其他任务且临时实验数据已清理。下一步等待资源空闲，并在 vLLM 启动前即时复查 GPU/端口后，以同一固定任务和 Qwen 重跑。当前本地完整测试基线 105 项及 3 个子用例通过，不能替代 M4。 |
